function [features, filter] = Dense_hog(input_image, window, hiPassFilter)
%DENSE_HOG compute dense HOG features from an image.
%modified to add high pass filtering optionally by bbt
% pass 0 for hiPassFilter to avoid filtering
%
...
% Kota Yamaguchi 2012
  ...
  %% added by bbt
  if hiPassFilter ~= 0
      hiPassFilter =  input_image<.75; %simple clipping of low intensity stuff --
%       hiPassFilter = repmat(hiPassFilter,1,1,31);
      filter = hiPassFilter;
%       fin = zeros(size(features));
%       fin(hiPassFilterMD)=features(hiPassFilterMD);
%       features = fin;
  else 
      filter = 0;
  end
