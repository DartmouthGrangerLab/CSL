...
function [vocab,VOCAB_SIZE] = BuildVocab_Concentricsubtraction(VOCAB_SIZE, QuantImg)
    allvals = [];
    for nFile = 1:numel(QuantImg)
        img = QuantImg{nFile};
        if numel(allvals) == 0
            allvals = reshape(img, 1, numel(img));
        else
            allvals = [allvals reshape(img, 1, numel(img))];
        end
    end
